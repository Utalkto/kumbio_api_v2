...
from communications.notifications import replace_message_tags
...
@celery_app.task()
def owner_wellcome_whatsapp(owner):
...
@celery_app.task()
def owner_wellcome_whatsapp_not_complete_onboarding(owner):
...
@celery_app.task()
def schedule_first_appointment(owner):
...
@celery_app.task()
def add_all_your_services(owner):
...
@celery_app.task()
def schedule_this_week_appointments(owner):
...
@celery_app.task()
def webpage_online_schedule(owner):
...
@celery_app.task
def premium_subscription_expiring(owner):
